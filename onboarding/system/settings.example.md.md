...
The file now teaches `ar-memory/system/` for internal mode, `ar-coordination/memory-repos/ar-<repo-name>/system/` for shared mode, v2 JSON examples, and a separate local coordinator scaffold.
...
| Shared memory settings use `memory-repo` storage under `ar-coordination/memory-repos/ar-<repo-name>/system/settings.json` and keep cross-repo policy in committed memory settings. | L45-L82 | [settings.example.md](agents-remember-md/system/settings.example.md) |
...
- 2026-05-11T19:42: Refreshed verification metadata to `aa85d3862bf21fed791e3170e6957f9288c319e8` after confirming the settings example still matches the split memory/coordination model.
