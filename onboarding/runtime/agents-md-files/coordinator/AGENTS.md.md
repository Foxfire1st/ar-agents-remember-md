...
| lastUpdated            | 2026-05-15T04:23+02:00                     |
...
is more restrictive. The memory-layer read path is now explicit: memory repos
are not expected to provide a root-level `AGENTS.md`; repo-specific guidance is
read from `system/settings.md`, `system/tools.md`, `system/sources.md`, and
...
It also preserves workflow approval boundaries by forbidding protected branch
...
| Coordinator defaults route agents to memory-layer `system/*` files and explicitly avoid a root-level memory-repo `AGENTS.md` expectation. | L47-L70   | [runtime/agents-md-files/coordinator/AGENTS.md](agents-remember-md/runtime/agents-md-files/coordinator/AGENTS.md) |
| Branch/worktree approval boundaries and memory-layer authority remain listed in the template. | L72-L88   | [runtime/agents-md-files/coordinator/AGENTS.md](agents-remember-md/runtime/agents-md-files/coordinator/AGENTS.md) |
