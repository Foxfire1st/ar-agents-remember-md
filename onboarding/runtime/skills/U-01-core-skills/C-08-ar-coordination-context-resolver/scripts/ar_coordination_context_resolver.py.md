...
| path                   | `runtime/skills/U-01-core-skills/C-08-ar-coordination-context-resolver/scripts/ar_coordination_context_resolver.py` |
...
The script defines dataclasses for storage, cross-repo entries, coordination selection, and coordination context; parses JSON-first settings and Markdown content; detects checkout versus installed coordinator runtime roots; detects internal or external selection; resolves memory, coordination, temporary artifact, and repo-specific task roots; uses coordination-root onboarding when memory-root onboarding is absent; loads task contract facts when present; checks current wrapper task roots before persisted `*-ar` roots when resolving by task name; resolves branch-gated cross-repo entries using actual checkout branches plus ledger commit metadata; and exposes a CLI.

### Conventions

All paths are normalized for portable output. The helper prefers explicit inputs, then installed-runtime or checkout configuration, then internal fallback. It keeps path eligibility and storage mode separate, exposes `code_repository_name` and `code_repository_root` as the resolver-facing code repository identity, exposes `task_root` as the repo task namespace unless a concrete task is supplied, and exposes `temp_root` as the coordination-local place for non-durable work artifacts.

### Invariants And Boundaries

The script must not mutate repositories, write onboarding files, create task folders, create worktrees, or repair memory ledgers. Its output is a contract consumed by C-02, C-05, W-02, and C-09.
...
| Installed runtime detection treats a root with `skills/`, `system/`, `memory-repos/`, and `tasks/` as the default coordination root, so non-`ar-coordination` install paths still resolve to themselves. | L133-L243 | [ar_coordination_context_resolver.py](agents-remember-md/runtime/skills/U-01-core-skills/C-08-ar-coordination-context-resolver/scripts/ar_coordination_context_resolver.py) |
| JSON settings parsing extracts storage mode, path rules, and `crossRepo.allow` from the same settings document.                                                                                                                          | L368-L396                | [ar_coordination_context_resolver.py](agents-remember-md/runtime/skills/U-01-core-skills/C-08-ar-coordination-context-resolver/scripts/ar_coordination_context_resolver.py) |
...
| Contract resolution checks current wrapper task roots first and persisted `*-ar` roots second when only a task name is supplied.                                                                                                         | L24-L40; L808-L825       | [ar_coordination_context_resolver.py](agents-remember-md/runtime/skills/U-01-core-skills/C-08-ar-coordination-context-resolver/scripts/ar_coordination_context_resolver.py) |
| Final context construction returns the repo-specific task namespace when no task name is supplied, keeps task-name resolution task-specific, and derives temp roots, worktree fields, ledger path, effective memory/docs/onboarding roots, sources, tools, and resolved cross-repo settings. | L1110-L1113; L1145-L1160 | [ar_coordination_context_resolver.py](agents-remember-md/runtime/skills/U-01-core-skills/C-08-ar-coordination-context-resolver/scripts/ar_coordination_context_resolver.py) |
...
- 2026-05-15T01:07+02:00: Updated after C-08's no-task-name `task_root` fallback was corrected to the repo-specific task namespace. Verification metadata remains pinned until closeout commits the source change.
- 2026-05-15T01:45+02:00: Updated after C-08 gained installed-runtime root detection for arbitrary coordinator directory names. Verification metadata remains pinned until closeout commits the source change.
