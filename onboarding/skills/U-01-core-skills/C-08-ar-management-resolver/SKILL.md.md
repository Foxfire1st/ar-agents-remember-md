...
This skill defines C-08, the authoritative facts-only resolver for memory roots, coordination roots, settings, path rules, task roots, worktree contract fields, ledger paths, and branch-gated cross-repo allowances.

## Code Commentary

### Logic

The skill tells agents to resolve context once and pass the resulting facts downstream instead of re-deriving topology in every workflow. It documents `coordination_root`, `memory_root`, optional contract/worktree fields, JSON-first settings, path rules, storage settings, and cross-repo v2 result states.
...
| Finding | Citations | Source Path |
| --- | --- | --- |
| The skill returns topology, repo roots, settings paths, task/docs/system roots, worktree fields, ledger path, path rules, and cross-repo data. | L25-L53 | [C-08 SKILL.md](agents-remember-md/skills/U-01-core-skills/C-08-ar-management-resolver/SKILL.md) |
...
| The implementation exposes the same fields through `ManagementContext`, context construction, and JSON output. | L81-L104; L1076-L1147; L1202-L1226 | [ar_management_resolver.py](agents-remember-md/skills/U-01-core-skills/C-08-ar-management-resolver/scripts/ar_management_resolver.py) |
