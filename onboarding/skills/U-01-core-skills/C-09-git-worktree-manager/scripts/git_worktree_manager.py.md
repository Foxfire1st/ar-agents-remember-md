...
The script loads C-08, resolves context, creates or loads worktree contracts, dry-runs or creates code worktrees, validates shared memory via `memory.md`, blocks incompatible memory with explicit choices, bootstraps clean-start shared memory repos, reports status, and performs approval-gated closeout with source-branch movement checks.

`bootstrap_memory_repo` treats an existing `memory.md` as authoritative and returns `already-ledgered` instead of overwriting it. For new baselines it initializes the memory repo if needed, configures a local Git identity when one is absent, creates the standard `onboarding`, `docs`, and `system` directories, adds `docs/.gitkeep` when `docs/` would otherwise be empty, commits memory content, then writes and commits the initial ledger.

### Conventions

`start` creates the code worktree first. Shared memory is prepared only when a compatible ledger exists or the user selects an explicit recovery path. Clean-start bootstraps the shared memory repo, prepares the recorded memory worktree, and records the new memory base commit. Baseline adoption can also call the bootstrap path directly through C-10. If the human selects disabled memory, the persisted contract is converted to disabled mode instead of retaining fake shared-memory paths. `closeout` refuses to run without `--approved` and stops if source branches moved since task start.

### Invariants And Boundaries

The script does not push. Shared-memory closeout commits code first, memory content second, and ledger updates third.

### Todos

Source branch movement checks are represented by base commits in contracts, but richer conflict analysis should be added before this becomes a production closeout tool.
