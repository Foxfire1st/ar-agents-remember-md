...
The skill now separates internal durable memory under `ar-memory/` from local coordination under `ar-coordination/`. It names the repository path input `code_repository_root`, tells agents to use C-08 for an active coordination context, keeps memory `system/` files together, and creates local coordination folders for system hints, memory repos, tasks, notes, and worktrees. Explicit shared scaffolding reads `agents-remember-md/.env` when present and otherwise falls back to the built-in `../ar-coordination` default; `.env.example` is documentation, not runtime input.

### Conventions

Default internal setup is local-first. Shared setup is explicit and creates coordination infrastructure plus per-repo shared memory repo settings under `memory-repos/ar-<code-repository-name>/system/`.
...
| Internal and explicit shared settings examples keep storage and path rules under memory-layer `system/settings.json`. | L122-L176 | [C-00 SKILL.md](agents-remember-md/skills/U-01-core-skills/C-00-initialize-coordination-root/SKILL.md) |
...
- 2026-05-12T18:51+02:00: Updated after the skill frontmatter moved to a lowercase skill name and explicit shared scaffolding stopped treating `.env.example` as runtime input.
